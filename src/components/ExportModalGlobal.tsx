...
interface ExportModalGlobalProps {
  mode: "entreprise" | "contact";
...
const ExportModalGlobal: React.FC<ExportModalGlobalProps> = ({
  mode,
  selectedCount,
  statsEntreprise,
  statsContact,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl p-0 relative">
        ...
        <div className="text-center text-[15px] text-gray-700 mb-6">
          Vous avez sélectionné <span className="font-bold">{selectedCount} contact(s)</span>.<br />
        ...
        <div className="flex flex-col sm:flex-row gap-4 px-6 mb-7">
          {/* Entreprises */}
          <div className={`flex-1 rounded-xl bg-white flex flex-col items-center py-6 min-w-[240px] border ${mode === "entreprise" ? "border-2 border-green-400 relative" : "border border-gray-200"}`}>
            {mode === "entreprise" && (
            ...
            <div className="flex flex-col items-center mb-2">
              <div className="bg-gray-100 rounded-full p-3 mb-2">
                <Building2 className={`w-7 h-7 ${mode === "entreprise" ? "text-green-500" : "text-gray-400"}`} />
              </div>
          ...
          {/* Contacts */}
          <div className={`flex-1 rounded-xl bg-white flex flex-col items-center py-6 min-w-[240px] border ${mode === "contact" ? "border-2 border-green-400 relative" : "border border-gray-200"}`}>
            {mode === "contact" && (
              <span className="absolute top-3 right-4 bg-white rounded-full border-2 border-green-400 p-1">
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="11" cy="11" r="10" stroke="#22C55E" strokeWidth="2" fill="#fff"/><path d="M7 11.5L10 14.5L15 9.5" stroke="#22C55E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </span>
            ...
            <div className="flex flex-col items-center mb-2">
              <div className="bg-green-100 rounded-full p-3 mb-2">
                <Users className={`w-7 h-7 ${mode === "contact" ? "text-green-500" : "text-gray-400"}`} />
              </div>
            ...
            <div className="mt-2 text-center w-full">
              ...
              <div className="flex flex-col gap-1 text-[13px] text-gray-600 w-full">
                <div className="flex items-center justify-center gap-1">
                  ...
                  <span className="font-bold ml-1">{statsContact.total.toLocaleString()}</span>
                </div>
                <div className="text-gray-500">dans <span className="font-bold">{statsContact.entreprises.toLocaleString()}</span> entreprises</div>
                <div className="mt-2 font-bold">Contacts directs</div>
                <div className="flex flex-col text-gray-500">
                  <span>{statsContact.contactsDirectEmail.toLocaleString()} avec Email</span>
                  <span>{statsContact.contactsDirectLinkedin.toLocaleString()} avec LinkedIn</span>
                </div>
                <div className="mt-2 font-bold">Contacts génériques</div>
                <div className="text-gray-500">{statsContact.contactsGeneriquesTel.toLocaleString()} avec téléphone</div>
              </div>
            </div>
          </div>
        </div>
...
export default ExportModalGlobal; 